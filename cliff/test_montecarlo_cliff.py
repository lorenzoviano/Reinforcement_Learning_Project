...
# ---------- hyper-parameters ----------
N_EPISODES        = 10000          # training duration
MAX_STEPS_ROLLOUT = 300            # cap for greedy episode
GAMMA             = 1.0
EPSILON_TRAIN     = 0.01           # ε during learning
...
def plot_path(xs, ys, chars, *, x_lo, x_hi, y_lo, y_hi):
    ...
    ax.plot(xs[m] + 0.5, ys[m] + 0.5, "-", lw=1.5, color="tab:blue", alpha=0.8, zorder=3)
    ax.scatter(xs[m] + 0.5, ys[m] + 0.5, s=20, color="tab:blue", alpha=0.8, zorder=4)
