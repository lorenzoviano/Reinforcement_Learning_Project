...
# ---------- hyper-parameters ----------
N_EPISODES        = 2000          # training duration
...
def plot_path(xs, ys, chars, *, x_lo, x_hi, y_lo, y_hi):
    ...
    ax.plot(xs[m] + 0.5, ys[m] + 0.5, "-", lw=1.5, color="tab:orange", alpha=0.8, zorder=3)
    ax.scatter(xs[m] + 0.5, ys[m] + 0.5, s=20, color="tab:orange", alpha=0.8, zorder=4)
