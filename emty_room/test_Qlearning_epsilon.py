#!/usr/bin/env python
"""
test_qlearning_epsilon.py

Runs off-policy Q-learning control with different exploration rates and
estimates how often the optimal policy (shortest-path, return = -8 in 5×5
...
from agents import QLearningOffPolicy      # ← Q-learning agent
from rl_task import RLTask
from tqdm import trange

# ------------- hyper-parameters -------------
N_EPISODES    = 500        # training episodes per run
N_TRIALS      = 50        # independent trainings per ε
...
MAX_STEPS_EVAL = 100      # cap for greedy rollout

def train_once(epsilon: float, seed: int | None = None) -> bool:
    """Train one Q-learning agent with given ε, return True if it finds optimal."""
    ...
    # instantiate QLearning with (epsilon, gamma, alpha)
...
def main():
    ...
    plt.title(
        f"Q-Learning Control on EMPTY_ROOM\n"
        f"α={ALPHA}, γ={GAMMA}, "
